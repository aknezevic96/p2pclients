...
##        logging.debug(str(history))
    ...
    def uploads(self, requests, peers, history):
        ...
        """
        """
        The reference client splits its upload bandwidth equally among the S slots,
        with each assigned the so-called equal-split rate.
        Optimistic unchoking helps a client to explore its neighborhood and find peers that will reciprocate
        and provide high upload bandwidth. Optimistic unchoking is also good for the system,
        helping peers that have just joined the swarm to obtain their first pieces
        before they have something they can reciprocate with.
        """
        # Symmetry breaking is good...
        ...
        if len(requests) == 0:
            ...
            bloks_recieved = dict(sorted(bloks_recieved.items(), key=operator.itemgetter(1),reverse=True))
            #Get the first 3 most upload peers 
            chosen = list(bloks_recieved.keys())[:3]
            #randomly choose a peer
            request = random.choice(requests)
            #added to chosen list
